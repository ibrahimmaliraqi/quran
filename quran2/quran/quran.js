class QuranPlayer {
    ...
    setupElements() {
        ...
        this.recitersSelect = document.getElementById('reciters-select');
    ...
    setupEventListeners() {
        ...
        // إضافة حدث تغيير القارئ من القائمة المنسدلة
        this.recitersSelect.addEventListener('change', () => this.handleReciterSelect());
        ...
        // إضافة استماع لأحداث شريط التقدم
        this.progressContainer.addEventListener('click', (e) => this.seek(e));
        this.progressContainer.addEventListener('mousemove', (e) => {
            if (e.buttons === 1) { // السحب مع زر الماوس الأيسر
                this.seek(e);
            }
        });
        
        // تحسين تجربة التحكم بالوقت
        this.audioPlayer.addEventListener('timeupdate', () => this.updateProgress());
    ...
    async loadReciters() {
        try {
            ...
            // تحديث قائمة القراء في select
            const recitersOptions = this.reciters
                .map(reciter => `<option value="${reciter.id}">${reciter.name}</option>`)
                .join('');
            
            this.recitersSelect.innerHTML = '<option value="">اختر القارئ...</option>' + recitersOptions;
            ...
            this.showError('لم نتمكن من تحميل قائمة القراء');
        }
    }
    ...
    handleReciterSelect() {
        const selectedId = this.recitersSelect.value;
        if (selectedId) {
            const reciter = this.reciters.find(r => r.id === parseInt(selectedId));
            if (reciter) {
                // تحديث حقل البحث
                this.reciterSearchInput.value = reciter.name;
                this.searchResults.style.display = 'none';
            ...
            // إعادة تعيين الحالة عند اختيار "اختر القارئ..."
            this.reciterSearchInput.value = '';
            this.resetPlayer();
        }
    }
    ...
    resetPlayer() {
        ...
        this.rewayaSelect.disabled = true;
        this.surahsSelect.disabled = true;
        this.rewayaSelect.innerHTML = '<option value="">اختر الرواية...</option>';
        this.updatePlayPauseIcon(false);
    }
    ...
    seek(event) {
        ...
        if (this.audioPlayer.duration) {
            this.audioPlayer.currentTime = clickPercentage * this.audioPlayer.duration;
            this.updateProgress();
        }
    }

    forward10Seconds() {
        if (this.audioPlayer.src && !isNaN(this.audioPlayer.duration)) {
